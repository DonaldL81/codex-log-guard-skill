...
- Codex 正在运行时启动延迟清理助手：运行 `tools/CodexLogGuardCli.ps1 deferred-clean`。
...
- 用户说“清理日志”“清理文件”时，先检查 Codex 是否正在运行；如果没有运行，调用 `clean`；如果正在运行，调用 `deferred-clean`，并告诉用户按助手窗口提示完全退出 Codex、清理后再重新打开 Codex。
